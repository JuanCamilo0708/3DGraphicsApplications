...
bool CmdPushRotationX::Execute(const std::vector<std::string>& params)
{
	...
	float deegrees = VariableCache::Get()->GetFloat(params[0]);
	MatrixStack::Get()->PushRotationX(deegrees * MathHelper::DegToRad);
	return true;
}
bool CmdPushRotationY::Execute(const std::vector<std::string>& params)
{
	...
	float deegrees = VariableCache::Get()->GetFloat(params[0]);
	MatrixStack::Get()->PushRotationY(deegrees * MathHelper::DegToRad);
	return true;
}
bool CmdPushRotationZ::Execute(const std::vector<std::string>& params)
{
	...
	float deegrees = VariableCache::Get()->GetFloat(params[0]);
	MatrixStack::Get()->PushRotationZ(deegrees * MathHelper::DegToRad);
	return true;
}
