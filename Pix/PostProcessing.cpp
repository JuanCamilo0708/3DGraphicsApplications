...
void PostProcessing::DrawPixel(int x, int y)
{
	...
	case EffectType::Mirror:
		color = mRenderTarget.GetColor(x, mRenderTarget.Getheight() - y);
		break;
	...
	case EffectType::Sepia:{
		...
		color.g = X::Math::Dot(colorVec, { 0.343f,0.686f,0.168f });
	...
	case EffectType::ChromaticAberration: {
		float offset = 0.01f;
		float yOffset = -0.05f;
		X::Color redChannel = mRenderTarget.GetColor(x + offset * x, y + offset * y);
		X::Color greenChannel = mRenderTarget.GetColor(x , y);
		X::Color blueChannel = mRenderTarget.GetColor(x - offset * x, y - offset * y);
